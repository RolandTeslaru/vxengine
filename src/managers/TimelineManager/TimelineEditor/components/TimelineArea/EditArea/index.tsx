...
import { handleCursorMutationByScale } from '../EditorCursor/utils';
...
import { handleCopyKeyframes, handleKeyframeDrag, handlePasteKeyframes, hydrateKeyframeKeysOrder } from './Keyframe/utils';
...
import { handleTrackDrag } from './Track/utils';

const startLeft = 22

export const EditArea = () => {
  const currentTimelineLength = useTimelineManagerAPI(state => state.currentTimelineLength);
  const [scale, searchQuery, trackTree, collapsedTrackNodes] = useTimelineEditorAPI(state => [state.scale, state.searchQuery, state.trackTree, state.collapsedTrackNodes])
    ...
    const unsubscribe = useTimelineEditorAPI.subscribe(
      ({ scale: currentScale }, { scale: prevScale }) => {
        if (currentScale !== prevScale) {
          ...
          keyframesMap.forEach((kfElement, keyframeKey) => {
          ...
          trackSegmentsMap.forEach((tsElement, key) => {
  ...
  const handleOnTrackSegmentMove = useCallback((
    e: DragEvent,
    ...
    handleTrackDrag(newLeft, prevLeft, trackKey, firstKeyframeKey, secondKeyframeKey, keyframesMap, trackSegmentsMap)
  }, [])

  const handleOnTrackSegmentMoveEnd = useCallback((e: MouseEvent) => {
    ...
    useTimelineManagerAPI.setState(
      produce((state: TimelineManagerAPIProps) => {
        selectedKeyframesFlatMap.forEach(selectKeyframeFlat => {
          ...
          const kfElement = keyframesMap.get(keyframeKey);
  ...
  const handleOnKeyframeMove = useCallback((
    e: DragEvent,
    deltaXRef: { current: number },
    trackKey: string,
    keyframeKey: string
    ...
    handleKeyframeDrag(newLeft, prevLeft, trackKey, keyframeKey, true, true, keyframesMap, trackSegmentsMap)
  }, [])



  const handleOnKeyframeMoveEnd = useCallback((
    e: DragEvent
  ) => {
    ...
    <>
      {filteredTree && Object.values(filteredTree).map((node, index) =>
        <TrackNode
          key={index}
          node={node}
          timelineClientWidth={timelineClientWidth}
          collapsedTrackNodes={collapsedTrackNodes}
          handleOnTrackSegmentMove={handleOnTrackSegmentMove}
          handleOnTrackSegmentMoveEnd={handleOnTrackSegmentMoveEnd}
          handleOnKeyframeMove={handleOnKeyframeMove}
          handleOnKeyframeMoveEnd={handleOnKeyframeMoveEnd}
        />
      )}
...
const TrackNode: React.FC<TrackNodeProps> = memo(({ 
  node, 
  timelineClientWidth, 
  collapsedTrackNodes, 
  handleOnTrackSegmentMove, 
  handleOnTrackSegmentMoveEnd,
  handleOnKeyframeMove,
  handleOnKeyframeMoveEnd
}) => {
  ...
  return (
    <>
      <div
        className={`relative border-t-[0.5px] border-b-[0.5px] border-neutral-800 ${!isTrack && "bg-neutral-950/90"}`}
        style={{
